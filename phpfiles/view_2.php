...
header('Access-Control-Allow-Origin: *');
	...
	$qrys = mysql_query("select *
		...
		$qry2 = mysql_query("SELECT (sum(final)/count(student)) as 'GPA' FROM rgrades where student='".$student."' and sem='2' and yl='1st' ");
	
	while($r2=mysql_fetch_array($qry2)){
			
			echo  '<h1 style="color:white;">GPA: '.number_format((float)$r2[0], 2, '.', '').'</h1>';
